...
rather than borrowing a service account. That is how it reads a secret you have
not given it yet.

> aside positive
>
> The deploy takes about seven minutes, and it runs while you work through the
> next four steps. A first deploy in a cold project is slower than every later one, because
...
Someone else has already turned it into a spec. Read `docs/spec.md`
...
repository at `.agents/skills/spec-adversary/SKILL.md`, which means
...
`.agents/skills/spec-adversary/SKILL.md` and read it. It is 150 lines and
...
writes it into every commit, and dispatch refuses a number that is not an open
issue.
...
The script prints two kinds of line, and they are not the same kind of claim.
`>` is the agent narrating itself, every file it reads and command it runs.
`+` is a commit that actually landed on the branch.
...
![pytest reporting every test passed on the agent's branch](docs/images/contract-tests-passing.png)
...
> is gone, so the script removes it first.
...
Run it a second time and it reports **Already torn down**, changing nothing.
Safe to re-run means no doubt about whether it finished.
